...
router.post("/update/:id", (request, responce) => {
  usersModel.findById(request.params.id, (error, userData) => {
    if (error) {
      ...
      if (request.body.image!= userData.image){
        toUpdate.image=request.body.image
      }
      if (request.body.tittle!= userData.tittle){
        toUpdate.tittle=request.body.tittle
      }
      if (request.body.detail!= userData.detail){
        toUpdate.detail=request.body.detail
      }
      if (request.body.discription!= userData.discription){
        toUpdate.discription=request.body.discription
      }
      if (request.body.longnitude!= userData.longnitude){
        toUpdate.longnitude=request.body.longnitude
      }
      if (request.body.lattitude!= userData.lattitude){
        toUpdate.lattitude=request.body.lattitude
      }
      
      if (request.body.reviews!= userData.reviews){
        toUpdate.reviews=request.body.reviews
      }
      if (request.body.rating!= userData.rating){
        toUpdate.rating=request.body.rating
      }
      if (
